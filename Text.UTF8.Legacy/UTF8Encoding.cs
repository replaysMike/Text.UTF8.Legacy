    ...
    public class UTF8Encoding : Encoding
    {
        ...
        */
        ...
        internal sealed class UTF8EncodingSealed : UTF8Encoding
        {
            ...
            public override ReadOnlySpan<byte> Preamble => _emitUTF8Identifier ? _preamble : Array.Empty<byte>();
        }

        // Used by Encoding.UTF8 for lazy initialization
        // The initialization code will not be run until a static member of the class is referenced
        internal static readonly UTF8EncodingSealed _default = new UTF8EncodingSealed(encoderShouldEmitUTF8Identifier: true);

        internal static readonly byte[] _preamble = new byte[3] { 0xEF, 0xBB, 0xBF };
        ...
        internal override void SetDefaultFallbacks()
        {
            ...
            if (_isThrowException)
            {
                _encoderFallback = EncoderFallback.ExceptionFallback;
                _decoderFallback = DecoderFallback.ExceptionFallback;
            }
            else
            {
                _encoderFallback = new EncoderReplacementFallback("\xFFFD");
                _decoderFallback = new DecoderReplacementFallback("\xFFFD");
            }
        }
                                              ...
                                              byte[] bytes, int byteIndex)
        {
            ...
            fixed (char* pChars = s)
            fixed (byte* pBytes = &System.Runtime.InteropServices.MemoryMarshal.GetReference((Span<byte>)bytes))
                                              ...
                                              char[] chars, int charIndex)
        {
            ...
            fixed (byte* pBytes = bytes)
            fixed (char* pChars = &System.Runtime.InteropServices.MemoryMarshal.GetReference((Span<char>)chars))
        ...
        public override unsafe String GetString(byte[] bytes, int index, int count)
        {
            ...
            fixed (byte* pBytes = bytes)
                return StringUtil.CreateStringFromEncoding(
        ...
        internal override unsafe int GetByteCount(char* chars, int count, EncoderNLS baseEncoder)
        {
            ...
            if (baseEncoder != null)
            {
                var encoder = (UTF8Encoder)baseEncoder;
                ch = encoder._surrogateChar;
                ...
                if (encoder.InternalHasFallbackBuffer)
                {
                    ...
                    if (fallbackBuffer.Remaining > 0)
                        throw new ArgumentException($"Argument_EncoderFallbackNotEmpty, {EncodingName}, {encoder.Fallback.GetType()}");
            ...
            for (; ; )
            {
                ...
                if (pSrc >= pEnd)
                {
                    ...
                    else
                    {
                        ...
                        if (fallbackBuffer != null && fallbackBuffer._bFallingBack)
                        {
            ...
            EncodeChar:
                ...
                if (InRange(ch, CharUnicodeInfo.HIGH_SURROGATE_START, CharUnicodeInfo.LOW_SURROGATE_END))
                {
                    ...
                    if (fallbackBuffer == null)
                    {
                        ...
                        // For fallback we may need a fallback buffer
                        if (baseEncoder == null)
                            fallbackBuffer = _encoderFallback.CreateFallbackBuffer();
                        else
            ...
            if (baseEncoder != null)
            {
                ...
                if (encoder.InternalHasFallbackBuffer)
                {
                    ...
                    if (fallbackBuffer.Remaining > 0 && encoder._throwOnOverflow)
                        throw new ArgumentException($"SR.Argument_EncoderFallbackNotEmpty, {EncodingName}, {encoder.Fallback.GetType()}");
        ...
        internal override unsafe int GetCharCount(byte* bytes, int count, DecoderNLS baseDecoder)
        {
            ...
            if (baseDecoder != null)
            {
                var decoder = (UTF8Decoder)baseDecoder;
                ch = decoder._bits;
            ...
            InvalidByteSequence:
                ...
                {
                    if (baseDecoder == null)
                        fallback = _decoderFallback.CreateFallbackBuffer();
                    else
        ...
        public override ReadOnlySpan<byte> Preamble =>
            ...
            _emitUTF8Identifier ? _preamble :
            Array.Empty<byte>();

        public override bool Equals(Object value)
        {
            var that = value as UTF8Encoding;
            if (that != null)
            {
